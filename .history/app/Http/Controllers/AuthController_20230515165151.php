...
class AuthController extends Controller
{
    ...
    public function anyProfile($username)
    {
        ...
        if ($username) {
         ...
         elseif($username){
            $owner = Owner::where('name', $username)->first();
            return new ownerResource($owner);
            return response()->json(['error' => 'Admin not found'], 404);
        }elseif($username){
            $planner = Planner::where('name', $username)->first();
            return new plannerResource($planner);
            return response()->json(['error' => 'Admin not found'], 404);
        }elseif($username){
            $supplier = Supplier::where('name', $username)->first();
