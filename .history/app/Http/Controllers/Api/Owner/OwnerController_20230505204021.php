...
class OwnerController extends Controller
{
    ...
    public function getAllHallsByPrice($minPrice,$maxPrice){
        ...
        if($halls){
                foreach($halls as $hall){
                    $m[]=$this->hallResources($hall);
            }
            return $this->response($m,"halls returned successfuly",200);
        }return $this->response('',"somthing wrong",401);
    }
    public function getAllHallsByName($name){
        $halls=Hall::where('verified', 'confirmed')->where('name',$name)->get();
        if($halls){
                foreach($halls as $hall){
                    $data[]=$this->hallResources($hall);
            }
