...
likes_data_hall = pd.read_sql_query("SELECT user_id, hall_id FROM likes", db)
booking_data_hall = pd.read_sql_query("SELECT user_id, hall_id FROM bookings", db)


# Check the column names of likes_data and booking_data DataFrames
if 'hall_id' not in likes_data.columns:
    print("Error: 'hall_id' column is missing in likes_data.")
    sys.exit(1)
if 'hall_id' not in booking_data.columns:
    print("Error: 'hall_id' column is missing in booking_data.")
    sys.exit(1)



print(likes_data.columns)
print(booking_data.columns)


user_item_matrix = pd.merge(likes_data, booking_data, on='user_id', how='outer')
user_item_matrix.columns = map(str.lower, user_item_matrix.columns)  # Convert column names to lowercase


# Perform data preprocessing and create a user-item matrix
# merged_data = pd.merge(likes_data, booking_data, on='user_id', how='outer')
# user_item_matrix = merged_data.groupby(['user_id', 'hall_id']).size().unstack().fillna(0)
...
# Generate recommendations for the target user based on similar users' preferences

# Get the hall details for the recommended halls
recommended_halls_details = halls_data[halls_data['hall_id'].isin(recommended_halls)]

# Convert the recommended halls to JSON format
recommended_halls_json = recommended_halls_details[['hall_id', 'hall_name']].to_json(orient='records')

# Print the recommended halls JSON
